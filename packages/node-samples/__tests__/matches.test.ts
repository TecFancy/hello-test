...
describe("Jest 匹配器示例", () => {
  ...
  // 匹配真值
  // 代码中的 `undefined`，`null`，`false` 有不同的含义，若测试时不想区分它们，可
  // 以用真值判断。
  test("真值判断", () => {
    const n = null;
    expect(n).toBeNull();
  ...
  test("捕获抛出的错误", () => {
    ...
    expect(() => withErr()).toThrow();
    expect(() => withErr()).toThrow(Error);
    expect(() => withErr()).toThrow("you are using the wrong JDK");
    expect(() => withErr()).toThrow(/JDK/);
    // expect(() => withErr()).toThrow(/^you are using the wrong JDK$/); // 不通过
    expect(() => withErr()).toThrow(
